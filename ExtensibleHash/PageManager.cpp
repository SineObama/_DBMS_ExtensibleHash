...
int VLRUtil::insert(const char *s) {
	return insert(s, strlen(s));
}

int VLRUtil::insert(const char *s, size_t size) {
...
int VLRUtil::get(char *s, size_t *length, index_t slot) const {
	if (slot >= *_n)
		return -1;
